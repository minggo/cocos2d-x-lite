...
public:
    ...
    void clear(const vector<Light *> &validLights, const vector<gfx::Buffer *> &lightBuffers, const vector<uint> &lightIndices);
    void recordCommandBuffer(gfx::Device *device, gfx::RenderPass *renderPass, gfx::CommandBuffer *cmdBuff);
    
private:
    void attach(RenderObject *renderObj, uint subModelIdx, gfx::Buffer *lightBuffer, uint lightIdx, Pass *pass, vector<MacroPatch> patches);

private:
    vector<vector<SubModel *>> _sortedSubModelsArray;
    vector<vector<PSOInfo>> _sortedPSOCIArray;
